  ...
  <div id="dayReconciliation">
    <Tabs v-model="name" type="card" @on-click="change">
      ...
      >
        ...
        <div>
          <DatePicker
            v-model="reconciliation.beginTime"
            :options="options1"
            type="month"
          ...
          <DatePicker
            v-model="reconciliation.endTime"
            :options="options2"
            type="month"
            placeholder="Select month"
            style="width:150px"
    ...
    <div id="lineMonth" style="width:100%;height:400px" />
  </div>
</template>
...
export default {
  ...
  methods: {
    getData() {
      ...
      this.$post("/api/reconMonth/info", obj).then(res => {
        console.log(res);
        if (res.code == 100) {
    ...
    getReconciliation() {
      ...
      obj.beginTime = this.DateTool.format(
        this.reconciliation.beginTime,
        "yyyy-MM-dd"
      );
      obj.endTime = this.DateTool.format(
        this.reconciliation.endTime,
        "yyyy-MM-dd"
      );
      console.log(obj);
    ...
    startCheck() {
      let self = this;
      if (!this.reconciliation.endTime) {
        this.options1 = {};
      } else {
        this.options1 = {
          disabledDate(date) {
            return (
              date && date.valueOf() > self.reconciliation.endTime.valueOf()
            );
          }
        };
      }
    ...
    endCheck() {
      let self = this;
      if (!this.reconciliation.beginTime) {
        this.options2 = {};
      } else {
        this.options2 = {
          disabledDate(date) {
            return (
              date && date.valueOf() < self.reconciliation.beginTime.valueOf()
            );
          }
        };
      }
    ...
    //画图
    getLine() {
      let dom = document.getElementById("lineMonth");
      console.log("dom", dom);
      var myChart = this.$echarts.init(document.getElementById("lineMonth"));
      myChart.clear();
