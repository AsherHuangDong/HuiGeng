<template>
  <div class="box">
    ...
    <el-col :span="8">
      审核状态
      <el-select v-model="queryParams.status" placeholder="请选择" style="width:200px" @change="search">
        <el-option
    ...
    <el-col :span="6">
      排序
      <el-select v-model="queryParams.orderBy" placeholder="请选择" style="width:200px" @change="search">
        <el-option
    ...
    <el-col :span="23" :offset="1">
      <el-table
        ...
        style="width: 100%"
      >
        <el-table-column prop="kycImgUrl" label="图片" min-width="20%">
          <!-- 图片的显示 -->
          <template slot-scope="scope">
            <img :src="scope.row.kycImgUrl" min-width="70" height="70" alt="图片">
          </template>
        </el-table-column>
        <el-table-column
          prop="userId"
...
<script>
import { KYCList } from '../../service/bg'
export default {
  name: 'KYC',
  data() {
    return {
  ...
  methods: {
    ...
    search() {
      ...
      const url = '/api/bg/kycAudit/all/' + this.pageParams.current + '/' + this.pageParams.size
    ...
    ispass(row, ispass) {
      const self = this
      this.$router.push({ name: '/auditClass/KYCAudit', params: { row: row, self: self, ispass: ispass }})
    }
  }
}
</script>
