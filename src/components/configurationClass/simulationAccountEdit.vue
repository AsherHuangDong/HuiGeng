...
export default {
  ...
  methods:{
    ...
    onSubmit() {
      ...
      this.$put('/api/bg/simuacc', this.form)
        .then(res => {
    ...
    getDetailsById() {
      this.$get('/api/bg/simuacc/' + this.row.id)
        .then(res => {
          console.log('/api/bg/simuacc', res)
          this.form.strategyId = res.data.info.strategyId
          let temp = res.data.info.historyProfit.substring(1,res.data.info.historyProfit.length - 1).split(",");
          let str = ''
          temp.forEach(item => {
            str +=  item.trim() + '\n'
          })
          this.form.historyProfit = str
          console.log('str', str)
      })
    }
  }
