...
export default {
  ...
  mounted() {
    ...
    this.form.strategyId = this.row.strategyId
    this.getStrategy()
  },
  methods: {
    getStrategy() {
      ...
      this.$get(url).then(res => {
        console.log(url, res)
        this.form.strategyName = res.data.info.strategyName
        this.form.strategyDetail = res.data.info.strategyDetail
        this.form.exchangeIdArr = res.data.info.exchangeIdArr
        this.form.startCost = res.data.info.startCost
        let tmp = []
        res.data.info.vtransactionPair.forEach(element => {
          tmp.push(element.transactionPairId)
          if(element.transactionPairId == 1){
            this.baozhengjin1 = element.minMargin
            this.chengzaijin1 = element.maxBearingCapital
          }
          if(element.transactionPairId == 2){
            this.baozhengjin2 = element.minMargin
            this.chengzaijin2 = element.maxBearingCapital
          }
          if(element.transactionPairId == 3){
            this.baozhengjin3 = element.minMargin
            this.chengzaijin3 = element.maxBearingCapital
          }
        });
        this.transactionPairIds = tmp.map(String)
      })
    },
    onSubmit() {
      ...
      this.$put('/api/bg/strategy', this.form)
        .then(res => {
