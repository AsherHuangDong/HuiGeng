...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      this.form.bannerTittle = this.$route.params.row.bannerTittle
      this.form.bannerDetail = this.$route.params.row.bannerDetail
      this.tmpstatus = this.$route.params.row.status == 1
      this.file = this.$route.params.row.bannerImgUrl
    },
    methods: {
      tirggerFile(event) {
      ...
      save() {
        ...
        this.$put('/api/bg/banner', form).then(res => {
            console.log(res.data)
            if(res.code == 100){
              this.$message({
                message: '编辑成功',
